...
const queryPromise = async <T = any>(query: string, values: any[] = []): Promise<T> => {
  try {
...
export const getTips = async (req: Request, res: Response) => {
  try {
    ...
    const formattedResults = results.map((tip: { signing: Buffer | null }) => ({
      ...tip,
      signing: tip.signing
...
export const insertTip = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const results = await queryPromise<{ id: number }[]>(query, [tip, github, githubURL, signingBuffer]);
